...
choose_database_and_do() {
    ...
    select bd in $(echo "SHOW DATABASES" |sudo -u $ADMIN_USER mysql -r -B -s) ; do
        test -n "$bd" || return 2
        eval "$* $bd"
        return $?
    done
...
_create_database() {
    ...
    echo "CREATE DATABASE $newdbname;" |sudo -u $ADMIN_USER mysql && \
...
new_user() {
    ...
    echo -e "CREATE USER '$username'@'localhost' IDENTIFIED BY '$password'" |sudo -u $ADMIN_USER mysql && \
...
grant_user() {
    ...
    echo -e "GRANT ALL PRIVILEGES ON $dbname.* TO '$username'@'localhost'; FLUSH PRIVILEGES;" |sudo -u $ADMIN_USER mysql && \
...
ls_databases() {
    sudo -u $ADMIN_USER mysqlshow|more
}


sql_shell() {
    dbname="$1"
    sudo -u $ADMIN_USER mysql $dbname
}
...
restore_database() {
    ...
    sudo -u $ADMIN_USER mysql "$dbname" < "$sqlfile" && \
...
remove_database() {
    ...
    echo "DROP DATABASE $dbname" |sudo -u $ADMIN_USER mysql && \
